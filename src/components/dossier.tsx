      ...
      <div className="mx-auto max-w-5xl px-6 sm:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {TECH_CATEGORIES.map((category) => (
            <div
              key={category.title}
              className="flex flex-col justify-between rounded-[8px] border border-[#EAEAEA] bg-[#FBFBFA] p-6 sm:p-7"
            >
              <div>
                ...
                <div className="space-y-4">
                  {category.skills.map((skill) => (
                    <div
                      key={skill.name}
                      className="border-t border-[#EAEAEA] pt-3 text-xs flex flex-col gap-1"
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-medium text-[#111111] font-sans">
                          {skill.name}
                        </span>
                        {skill.kbd && <kbd>{skill.kbd}</kbd>}
                      </div>
